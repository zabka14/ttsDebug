...
{
if(lockpicking)exitwith{player groupchat "vous avez deja crocheter ce  véhicule"};
...
if (not(_closest == -1)) then 
	...
	if(_closeVcl isKindOf "Tank" || _closeVcl isKindOf "Air")exitwith{player groupchat "Vous ne pouvez pas crocheter ce vehicule!"};
	if (_closeVcl in INV_VehicleArray) then 
		...
		if ((random 100) < lockpickchance) then 
			...
			(format ["%1 switchmove ""%2"";", player, "repairingkneel"]) call broadcast;
				sleep 3;
				_car say "carallarm" call broadcast;
				sleep 3;
