...
"if (iscop) then {hint ""Le camion d'approvisionnelent partira dans quelques minutes ."";}" call broadcast;
...
(if(iscop)then{[WEST,"HQ"] sidechat "Le camion d'approvisionnement demande une escorte."}) call broadcast; 
...
while {true} do 
	...
	if ((!(alive konvoysoldat)) and (_soldatvar == 0)) then 
		...
		"hint localize ""STRS_konvoy_hijacked""; konvoytruck setVehicleLock ""Deverrouiller"";" call broadcast;
	...
	if(konvoytruck distance Militarybase < 400 and _counter > 60 and speed konvoytruck == 0)exitwith{"hint ""Le camion d'approvisionnement est revenu à la base."";" call broadcast;};
