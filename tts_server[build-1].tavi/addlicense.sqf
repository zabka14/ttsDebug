...
if (_art == "add") then 
	...
	if(_license == "mafial" and streetrep < 15) exitWith { player groupChat "Votre réputation de la rue n'est pas assez élevé, vous avez besoin de 15 points."};
	if(_license == "mafial" and !(isciv)) exitWith { player groupChat "Vous devez etre un Civil!"};
	if(_license == "mayorlic" and !(ismayor)) exitWith { player groupChat "Restriced to gouverneur"};
    ...
    if(_license == "pmcl" and !(_uid in specarray)) exitWith { player groupChat "Restriced to PMC"};
    if(_license == "millis" and !(_uid in specarray)) exitWith { player groupChat "Restriced to NG's"};
	if(_license == "bounh" and !(_uid in specarray) and !(isciv)) exitWith { player groupChat "Restriced Chasseur de Prime"};
