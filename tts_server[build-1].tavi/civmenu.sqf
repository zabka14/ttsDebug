...
if (_civkopfgeld != 0) then 
	...
	(format['if(rolestring == "%1")then{kontostand = kontostand + %2; player groupChat "Ce civil avais un prime de $%2! Vous gagnez cette prime!"}', _copobj, _civkopfgeld]) call broadcast;
...
while {true} do 
	...
	hintsilent format["Temps jusqu'à la libération: %1 secondes\nCaution: $%2", _timetotake, round(_bounty)];
...
while {true} do 
	...
	hintsilent format["Temps jusqu'à la libération: %1 secondes\ncautions: $%2", _timetotake, round(_bounty)];
...
{
(format ['if (rolestring == "%1") then {hint "%2 a été volé récemment";};', _aktionsStarter, player]) call broadcast;	
};
_geld  = 'geld' call INV_GetItemAmount;
_amounttosteal = (floor(random _geld));
["geld", -(_amounttosteal)] call INV_AddInvItem;

(format ['if (rolestring == "%1") then {["geld", %2] call INV_AddInvItem;};hint "%1 vole %2 de %3";',_aktionsStarter, _amounttosteal, player]) call broadcast;
