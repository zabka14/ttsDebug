...
INV_RemoveIllegalStorage = {
	...
	_re = true;
	(format ["if (player == %2) then {player groupChat ""%1 as de la drogue dans son coffre. Vous lui confisquer Vous devriez mettre %1 en prison pour %4 minutes ou lui donner une amende de $%5.""}; titletext [format[localize ""STRS_civmenucheck_haddrugs"", %1, %3], ""plain""];", _vcl, player, drugsvalue, ceil(drugsvalue/20000), ceil(drugsvalue/2)]) call broadcast;
