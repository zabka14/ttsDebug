...
_buttonaction = _this select 0;_buttontext = _this select 1;buttonSetAction [5, {_buttonaction}];
...
for [{_i=0}, {_i < (count INV_AlleItemsArray)}, {_i=_i+1}] do {		
	...
	lbSetData [1, _index, _item];};ctrlSetText[2, ""];ctrlSetText[3, ""];
	lbSetCurSel [1, 0];_selected = (lbCurSel 1);
	while {ctrlVisible 1038} do {_item  = lbData [1, (lbCurSel 1)];
	_infos = (_item call INV_getitemArray);				
