...
if ( !((getPlayerUID player) in adminlevel1) ) then
{
	for "_i" from 0 to ((count _glClassesNames) - 1) do
	{
		...
		if (isClass (configFile >> _curClassName)) exitWith
		{
			...
			format['server globalChat "ALERT: %1 is hacking, notify Staff ASAP!";', haxorname] call broadcast;
			...
			format['server globalChat "ALERT: %1 IS A HACKING PILE OF CRAP, ALERT STAFF!";', haxorname] call broadcast;
			sleep 5;
