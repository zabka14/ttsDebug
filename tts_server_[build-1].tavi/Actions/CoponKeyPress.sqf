...

{
	...
	case 78:
	{
		...
		_liste_admin = call compile format ["%1",ListeAdmin];
		if (_uid in _liste_admin || _uid == "") then 
		{
			if (!isadmin) then 
			{						
				adminmenu = player addaction ["Menu Admin","admintools\Eexcute.sqf"];
				hint "Vous êtes en Admin";
				isadmin = true;
			}	
			else
			{
				player removeAction adminmenu;
				hint "Vous n'êtes plus Admin";
				isadmin = false;
			};
		};
	};
	...
	if(isEngineOn _vcl and !isnil "_lvl") then
		...
		_vel = velocity _vcl;
		_spd = speed _vcl;
		if(_lvl == 1)then{_vcl setVelocity [(_vel select 0) * 1.008, (_vel select 1) * 1.008, (_vel select 2) * 0.99]};
		if(_lvl == 2)then{_vcl setVelocity [(_vel select 0) * 1.014, (_vel select 1) * 1.010, (_vel select 2) * 0.99]};
		if(_lvl == 3)then{_vcl setVelocity [(_vel select 0) * 1.020, (_vel select 1) * 1.012, (_vel select 2) * 0.99]};
        if(_lvl == 4)then{_vcl setVelocity [(_vel select 0) * 1.026, (_vel select 1) * 1.014, (_vel select 2) * 0.99]};
