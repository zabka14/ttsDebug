...
sleep 1;					
if(ischief)then{player groupchat format["Comme vous etes un chef de la police vous obtenez un chèque supplémentaire de $%1.", (chiefExtraPay call ISSE_str_IntToStr)]};   
...
if ((alive player) and (deadcam == 0)) then 
	...
	if (isMayor) then 
		...
		player groupchat format["Comme vous etes le gouverneur vous obtenez un chèque supplémentaire de $%1. Vous avez également $%2 de taxe.", (MayorExtraPay call ISSE_str_IntToStr), (MayorSteuern call ISSE_str_IntToStr)];		
