...
private ["_menge"];
if (shopactivescript == 1) exitwith {player groupchat "script déjà actif"};
	...
	if (_infos call INV_getitemIsIllegal and _cost > 0 and _infos call INV_getitemKindOf == "drug") then
		...
		for "_c" from 0 to (count gangsarray - 1) do 
			...
			if(_control == _gang and count _members > 0)then
				...
				format['if(name player in %1)then{player groupchat "vous avez reçu $%2 pour la drogue vendu"; kontostand = kontostand + %2};', _members, _income] call broadcast;
...
if(_menge <= 0)exitwith{player groupchat "La boutique a atteint son stock maximum pour cet article / véhicule"};
...
if(maxstock)then{player groupchat "La boutique a atteint son stock maximum pour cet article / véhicule"};
