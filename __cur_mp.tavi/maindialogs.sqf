...
if (_art == "spielerliste") then 
{
	...
	lbAdd [1, format ["Temps de connexion: %1 minutes", (round(time/60))]];		
	lbAdd [1, _trennlinie];
	lbAdd [1, localize "STRS_statdialog_playerinfo"];
	lbAdd [1, format ["Argent en poche: $%2", localize "STRS_statdialog_health", (('geld' call INV_GetItemAmount) call ISSE_str_IntToStr)]];
	lbAdd [1, format ["Argent en banque: $%2", localize "STRS_statdialog_health", kontostand]];
	...
	lbAdd [1, format ["%1: %2 seconds",    "Temps de respawn", round(DeadWaitSec+extradeadtime)]];
	if(isciv)then{lbAdd [1, format ["%1: %2",    "Les points d'inaptitude", demerits]]};
	...
	lbAdd [1, _trennlinie];
	lbAdd [1, "Les membres de la mafia:"];
	...
	lbAdd [1, "Argent dans la Banque:"];
	lbAdd [1, (format ["Est. le total des fonds dans le coffre principal de la banque: $%1", robpoolsafe1])];
	lbAdd [1, "Fonds sûrs de Goodfella:"];
	lbAdd [1, (format ["Est. le total des fonds dans le coffre-fort de banque Goodfellas: $%1", mafiabank])];
	if(!local_useBankPossible)then{lbAdd [1, (format ["Temps de verrouillage de la Banque reste: %1 secondes.", round rblock])]};		
	...
	lbAdd [1, format["Transferer: %1", bank_steuer] ];

	lbAdd [1, _trennlinie];

	lbAdd [1, "D E M A N D E (Oil):"];
	_demand = (((tankencost - 100)/200)*100);	
	lbAdd [1, format["Baril de pétrole: %1", _demand ]];
