...
if(!isnull (nearestobjects[getpos player,["EvMoney","Suitcase"], 1] select 0))exitwith{player groupchat "Vous ne pouvez pas déplacer des éléments sur le dessus de l'autre. déplacer et essayer à nouveau."};

if (_amount <= 0) exitwith {format["hint ""%1 a laché %2!"";", (name player), _amount] call broadcast;};
