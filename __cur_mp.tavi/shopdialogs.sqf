...
_CopOnlyShops = [copair,copcar,copcar1,copbasic,coppatrol,copsheriff,copswat,copbasic2,copboatshop,tdoc,copk9cars,coptrafficcars,coptraffic,copvicecars,copswatcars,copsub,copair1,cgweapons,cgboatshop];
...
if (_shop in _CopOnlyShops) then {
    ...
    if (_PlayerTeam == "CIV") exitWith {player groupchat "Les civils ne peuvent pas utiliser cette boutique"; closedialog 0};
    };
if(_shop in _donoro and !((getPlayerUID player) in donatorlevel4)) exitwith {player groupchat "Sortez d'ici, vous n'êtes pas un donateur!"; closedialog 0};
if(_shop in _VIPcop and !((getPlayerUID player) in donatorlevel4) and (_PlayerTeam == "CIV")) exitwith {player groupchat "Sortez d'ici, vous n'êtes pas un donateur!"; closedialog 0};
if(_shop in _pmcs and !((getPlayerUID player) in specarray)) exitwith {player groupchat "Vous n'êtes pas PMC!"; closedialog 0};
if(_shop in _milps and !((getPlayerUID player) in specarray)) exitwith {player groupchat "C'est le QG de garde national!"; closedialog 0};
if(_shop in _ADMIN and !((getPlayerUID player) in adminlevel4)) exitwith {player groupchat "Vous n'êtes pas administrateur!"; closedialog 0};
if(_shop in _UN and !((getPlayerUID player) in specarray)) exitwith {player groupchat "Vous n'avez pas le droit!"; closedialog 0};
if(_shop in _SD and !((getPlayerUID player) in superdonatorarray)) exitwith {player groupchat "Vous n'êtes pas un super Donateur"; closedialog 0};
...
while {ctrlVisible 1015} do {
	...
	if (_cursel >= 0) then {
		...
		if (_infos call INV_getitemType == "item") then {
			ctrlSetText [3, format ["Acheter ($%1, %2kg)", _moneyanzeige, (_slider*(_infos call INV_getitemTypeKg))]];
		} else {
			ctrlSetText [3, format ["Acheter ($%1)", _moneyanzeige]]; 
		};
	} else {
		ctrlSetText [3,  "/"];
	};
