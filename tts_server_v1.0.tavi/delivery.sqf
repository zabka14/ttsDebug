...
if(canceltime) exitwith
	...
	player groupchat "Vous ne pouvez pas commencer une autre mission de livraison si tôt après avoir échoué à la precedente!";
...
while {true} do 
	...
	if (vehicle player == player and _a1 == 0) then 	
		...
		vcl removeaction deliveryinfoaction;
		deliveryinfoaction = player addAction ["Delivery mission info", "noscript.sqf", 'hint format["Destination: %1\nTemps restant: %2 secondes.\nPay: $%3\nDistance restante: %4m", workplacejob_deliveryname, round(maxtime - timetaken), round workplacejob_deliverymoney, (round(workplacejob_deliveryflag Distance player))]',1,false,true,"",""]; 
...
vcl removeaction deliveryinfoaction;
player groupchat "Mission de livraison echoué!";
alreadygotaworkplacejob = 0;
workplacemissioninfos = ["", ""];
