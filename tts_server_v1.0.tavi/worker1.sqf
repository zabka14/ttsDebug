...
if (slavescriptrunning == 1) exitwith {player groupchat "Scripts déjà en cours.."};
slavescriptrunning = 1;		
['geld', -(huren_cost)] call INV_AddInvItem;

player groupChat "Vous venez d'acheter une pute, envoyer la à un Hotel pour la faire travailler !";
...
while {true} do 
	...
	for [{_i=0}, {_i < 120}, {_i=_i+1}] do 
		...
		if ( (not(alive _hurenname)) or (not(alive player)))  exitWith {sleep 2; if(!isnull _hurenname)then{player groupChat localize "STRS_huren_tot"}else{player groupChat "Une de vos putes vous a balancer et est rentrée chez elle, Vous etes maintenant recherché pour proxénétisme."}; localhuren = localhuren - 1; _hurenname SETDAMAGE 1; _exitvar = 1;};
